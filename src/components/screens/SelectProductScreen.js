...
const SelectProductScreen = ({ products, onProductSelect, onLogout, clientEmail, onViewCart, cartCount, onViewOrders }) => (
  <ScrollView style={styles.container}>
    ...
    {products.map(option => (
      <View key={option.id} style={styles.card}>
        ...
        <AppButton label="Adicionar ao Carrinho" onPress={() => onProductSelect(option)} />
      </View>
    ))}
    {/* Atalho para revisar itens do carrinho */}
    <AppButton label={`🛒 Ver Carrinho (${cartCount})`} variant="outline" onPress={onViewCart} style={{ marginHorizontal: 15 }} textStyle={{ color: '#007BFF' }} />
    <AppButton label="📦 Ver Pedidos" variant="outline" onPress={onViewOrders} style={{ marginHorizontal: 15 }} textStyle={{ color: '#28A745' }} />
    {/* Encerra a sessão do cliente */}
    <AppButton label="Sair" variant="secondary" onPress={onLogout} style={{ marginHorizontal: 15 }} />
  </ScrollView>
);
